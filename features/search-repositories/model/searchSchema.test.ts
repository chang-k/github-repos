...
describe('searchSchema', () => {
  describe('keyword', () => {
    it('空文字列の場合、エラーになる', () => {
      ...
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('キーワードを入力してください');
      }
    });
  ...
  describe('star', () => {
    ...
    it('負の数の場合、エラーになる', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('0以上の整数のみ記入してください');
      }
    });
    ...
    it('max < min の場合、エラーになる', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('最大値は最小値以上である必要があります');
      }
    });
  ...
  describe('duration', () => {
    ...
    it('YYYY-MM-DD 以外の形式の場合、エラーになる', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('日付は YYYY-MM-DD 形式で入力してください');
      }
    });
    ...
    it('pushed < created の場合、エラーになる', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('最終更新日は作成日以降である必要があります');
      }
    });
